        ...
        if (request()->isPost()) {
            try {
                ...
                $s_h = "select trim(a.work_order_no) work_order_no,a.work_id,
                        ...
                        trim(a.sender_name) sender,a.gzdnr,a.sign_note,a.sign_id
    ...
    function lb_workorder()
    {
        if (request()->isPost()) {
            ...
            try {
                ...
                //计划日期
                $date = input('param.plan_date');

                //查询某天派单
                $sql = "select trim(a.work_order_no) work_order_no,a.work_id,
                        a.plan_date,a.sender_date,a.status,
                        a.work_flag,a.mcht_name,a.addr,a.linkman,a.telno,
                        a.work_date,a.sign_date,a.sign_name,
                        a.device_type,a.work_name,a.mcht_addr,
                  ...
                  from v_workorder_1 a
                  where 1 = 1 
                    and a.work_id = '{$work_id}' and a.plan_date = '{$date}'
    ...
    function details_workorder()
    {
        if (request()->isPost()) {
            try {
                ...
                $sql = "select a.sys_date,trim(a.work_order_no) work_order_no,trim(a.term_no) term_no,
                  a.sender_name,a.mcht_name2,a.status,a.status_name,
          ...
          where 1 = 1
              and a.work_order_no = '{$work_order_no}'";
                ...
                if ($data) {
                    ...
                    $gongzuodan = '';
                    if ($data['work_date']) {
                        $gongzuodan = Db::table('workorder_es')->field($field)->where(['WORK_ORDER_NO' => $work_order_no, 'FILETYPE' => '0'])->order('create_date desc')->find();
                    } else {
                        $where = "";
